...
export const FORK_UPDATE_REPO =
	process.env === undefined ? '' : (process.env.FLUXER_FORK_UPDATE_REPO ?? '');
	...
	return getManualDownloadFormatPreference()
		...
		.map((format) => {
			...
			return {
				...
				url:
					FORK_UPDATE_REPO.length > 0 && file?.url
						? file.url
						: buildManualVersionDownloadUrl(info.version, format),
